...
interface_pipe = subprocess.Popen(
    ['grep', IFACE], stdin=ip_pipe.stdout, stdout=subprocess.PIPE)
IP_ADDRESS = subprocess.check_output(
    ['cut', '-d', ' ', '-f', '9'], stdin=interface_pipe.stdout).split()[0]
...
class CaptivePortal(SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        subprocess.call(["iptables", "-t", "nat", "-I",
                        "PREROUTING", "1", "-s", remote_IP, "-j", "ACCEPT"])
        subprocess.call(["iptables", "-I", "FORWARD",
                        "-s", remote_IP, "-j", "ACCEPT"])
...
print ".. Allow TCP DNS"
subprocess.call(["iptables", "-A", "FORWARD", "-i", IFACE,
                "-p", "tcp", "--dport", "53", "-j", "ACCEPT"])
print ".. Allow UDP DNS"
subprocess.call(["iptables", "-A", "FORWARD", "-i", IFACE,
                "-p", "udp", "--dport", "53", "-j", "ACCEPT"])
print ".. Allow traffic to captive portal"
subprocess.call(["iptables", "-A", "FORWARD", "-i", IFACE, "-p",
                "tcp", "--dport", str(PORT), "-d", IP_ADDRESS, "-j", "ACCEPT"])
subprocess.call(["iptables", "-A", "FORWARD", "-i", IFACE, "-p",
                "tcp", "--dport", str(RE_PORT), "-d", IP_ADDRESS, "-j", "ACCEPT"])
print ".. Allow traffic to F-Droid repositories"
subprocess.call(["iptables", "-A", "FORWARD", "-i", IFACE, "-p",
                "tcp", "--dport", "80", "-d", FDROID_IP_1, "-j", "ACCEPT"])
subprocess.call(["iptables", "-A", "FORWARD", "-i", IFACE, "-p",
                "tcp", "--dport", "80", "-d", FDROID_IP_2, "-j", "ACCEPT"])
...
subprocess.call(["iptables", "-t", "nat", "-A", "PREROUTING", "-i", IFACE, "-p", "tcp",
                "--dport", "80", "-j", "DNAT", "--to-destination", IP_ADDRESS+":"+str(RE_PORT)])
